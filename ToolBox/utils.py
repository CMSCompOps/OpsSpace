...
def check_ggus(ticket):
    conn = httplib.HTTPSConnection('ggus.eu', cert_file=os.getenv('X509_USER_PROXY'),
...
def listDelete(url, user, site=None):
    ...
    return list(itertools.chain.from_iterable(
        [(subitem['name'], item['requested_by'], item['id']) for subitem in item['node'] if
         subitem['decision'] == 'pending'] for item in items))
...
class unifiedConfiguration:
    def __init__(self):
        self.configs = json.loads(
            open('/afs/cern.ch/user/c/cmst2/Unified/WmAgentScripts/unifiedConfiguration.json').read())
...
class componentInfo:
    ...
    def check(self):
        ...
        try:
            ...
            cust = findCustodialLocation('cmsweb.cern.ch',
    ...
    def tell(self, c):
        sendEmail("%s Component Down" % c, "The component is down, just annoying you with this", "vlimant@cern.ch",
                  ['vlimant@cern.ch', 'matteoc@fnal.gov'])
...
def notRunningBefore(component, time_out=60 * 5):
    ...
    while True:
        if time_out < 0:
            ...
            return False
        process_check = filter(None,
                               os.popen('ps -f -e | grep %s.py | grep -v grep  |grep python' % component).read().split(
                                   '\n'))
...
def duplicateLock(component=None):
    ...
    if len(process_check) > 1:
        ...
        sendEmail('overlapping %s' % component,
                  'There are %s instances running %s' % (len(process_check), '\n'.join(process_check)))
    ...
    def __init__(self):
        ...
        self.cache['ssb_106'] = {
            ...
            'getter': lambda: json.loads(os.popen(
                'curl -s --retry 5 "http://dashb-ssb.cern.ch/dashboard/request.py/getplotdata?columnid=106&batch=1&lastdata=1"').read())[
                'csvdata'],
            'cachefile': None,
            'default': []
        }
        self.cache['ssb_107'] = {
            ...
            'expiration': default_expiration(),
            'getter': lambda: json.loads(os.popen(
                'curl -s --retry 5 "http://dashb-ssb.cern.ch/dashboard/request.py/getplotdata?columnid=107&batch=1&lastdata=1"').read())[
                'csvdata'],
            'cachefile': None,
            'default': []
        }
        self.cache['ssb_108'] = {
            ...
            'expiration': default_expiration(),
            'getter': lambda: json.loads(os.popen(
                'curl -s --retry 5 "http://dashb-ssb.cern.ch/dashboard/request.py/getplotdata?columnid=108&batch=1&lastdata=1"').read())[
                'csvdata'],
            'cachefile': None,
            'default': []
        }
        self.cache['ssb_109'] = {
            ...
            'expiration': default_expiration(),
            'getter': lambda: json.loads(os.popen(
                'curl -s --retry 5 "http://dashb-ssb.cern.ch/dashboard/request.py/getplotdata?columnid=109&batch=1&lastdata=1"').read())[
                'csvdata'],
            'cachefile': None,
            'default': []
        }
        self.cache['ssb_136'] = {
            ...
            'expiration': default_expiration(),
            'getter': lambda: json.loads(os.popen(
                'curl -s --retry 5 "http://dashb-ssb.cern.ch/dashboard/request.py/getplotdata?columnid=136&batch=1&lastdata=1"').read())[
                'csvdata'],
            'cachefile': None,
            'default': []
        }
        self.cache['ssb_158'] = {
            ...
            'expiration': default_expiration(),
            'getter': lambda: json.loads(os.popen(
                'curl -s --retry 5 "http://dashb-ssb.cern.ch/dashboard/request.py/getplotdata?columnid=158&batch=1&lastdata=1"').read())[
                'csvdata'],
            'cachefile': None,
            'default': []
        }
        self.cache['ssb_159'] = {
            ...
            'expiration': default_expiration(),
            'getter': lambda: json.loads(os.popen(
                'curl -s --retry 5 "http://dashb-ssb.cern.ch/dashboard/request.py/getplotdata?columnid=159&batch=1&lastdata=1"').read())[
                'csvdata'],
            'cachefile': None,
            'default': []
        }
        self.cache['ssb_160'] = {
            ...
            'expiration': default_expiration(),
            'getter': lambda: json.loads(os.popen(
                'curl -s --retry 5 "http://dashb-ssb.cern.ch/dashboard/request.py/getplotdata?columnid=160&batch=1&lastdata=1"').read())[
                'csvdata'],
            'cachefile': None,
            'default': []
        }
        ...
        self.cache['gwmsmon_site_summary'] = {
            ...
            'expiration': default_expiration(),
            'getter': lambda: json.loads(
                os.popen('curl --retry 5 -s http://cms-gwmsmon.cern.ch/totalview//json/site_summary').read()),
            'cachefile': None,
            'default': {}
        }
        ...
        self.cache['mss_usage'] = {
            ...
            'expiration': default_expiration(),
            'getter': lambda: json.loads(os.popen(
                'curl -s --retry 5 http://cmsmonitoring.web.cern.ch/cmsmonitoring/StorageOverview/latest/StorageOverview.json').read()),
            'cachefile': None,
            'default': {}
        }
                ...
                if not cache['data']:
                    ...
                    if os.path.isfile(cache['cachefile']):
                        ...
                        open(cache['cachefile'], 'w').write(
                            json.dumps({'data': cache['data'], 'timestamp': cache['timestamp']}, indent=2))
...
class siteInfo:
    def __init__(self):
        ...
        self.sites_eos = [s for s in self.sites_ready if
                          s in ['T2_CH_CERN', 'T2_CH_CERN_AI', 'T2_CH_CERN_T0', 'T2_CH_CERN_HLT']]
        ...
        self.sites_with_goodIO = ["T2_DE_DESY", "T2_DE_RWTH",
                                  ...
                                  ##"T2_UK_SGrid_RALPP",
                                  "T2_US_Caltech", "T2_US_MIT", "T2_US_Nebraska", "T2_US_Purdue", "T2_US_UCSD",
                                  "T2_US_Wisconsin", "T2_US_Florida",  # "T2_US_Vanderbilt",
                                  "T2_BE_IIHE",
                                  "T2_EE_Estonia",
                                  "T2_PL_Swierk",
        ...
        allowed_T2_for_transfer = ["T2_DE_RWTH", "T2_DE_DESY",
                                   ...
                                   "T2_US_Nebraska", "T2_US_Wisconsin", "T2_US_Purdue", "T2_US_Caltech",
                                   "T2_US_Florida", "T2_US_UCSD", "T2_US_MIT",
                                   "T2_BE_IIHE",
                                   "T2_EE_Estonia",
                                   "T2_CH_CERN",
                                   ]
    ...
    def usage(self, site):
        try:
            info = json.loads(os.popen(
                'curl -s "http://dashb-cms-job.cern.ch/dashboard/request.py/jobsummary-plot-or-table2?site=%s&check=submitted&sortby=activity&prettyprint"' % site).read())
            return info
        except:
            return {}
    ...
    def getRemainingDatasets(self, site):
        ...
        for line in os.popen(
                        'curl -s http://t3serv001.mit.edu/~cmsprod/IntelROCCS/Detox/result/%s/RemainingDatasets.txt' % site).read().split(
                "\n"):
        ...
        for (site, info) in _info_by_site.items():
            ...
            if key_for_cpu in info and site in self.cpu_pledges and info[key_for_cpu]:
                if self.cpu_pledges[site] < info[key_for_cpu]:
                    ...
                    if talk: print site, "could correct", info[key_for_cpu], "instead of", self.cpu_pledges[
                        site], "for CPU"
                    self.cpu_pledges[site] = int(info[key_for_cpu])
...
phdF</th><th>Priority</th></tr></thead>'
    ...
    def one_line(self, wf, wfo, count):
        ...
        order = ['percentage', 'acdc', 'duplicate', 'correctLumis', 'missingSubs', 'phedexReqs', 'dbsFiles',
                 'dbsInvFiles', 'phedexFiles']
        ...
        for out in self.record[wf]['datasets']:
            ...
            for f in order:
                if f in self.record[wf]['datasets'][out]:
                    ...
                    value = "-NA-"
                if f == 'acdc':
                    text += '<td><a href=https://cmsweb.cern.ch/couchdb/reqmgr_workload_cache/_design/ReqMgr/_view/byprepid?key="%s">%s</a></td>' % (
                    tpid, value)
                else:
                    text += '<td>%s</td>' % value
...
""")
        ...
        short_html.write(
            'Last update on %s(CET), %s(GMT), <a href=logs/checkor/last.log target=_blank> log</a> <a href=logs/recoveror/last.log target=_blank> postlog</a> <br>' % (
            time.asctime(time.localtime()), time.asctime(time.gmtime())))
        html.write(
            'Last update on %s(CET), %s(GMT), <a href=logs/checkor/last.log target=_blank> log</a> <a href=logs/recoveror/last.log target=_blank> postlog</a><br>' % (
            time.asctime(time.localtime()), time.asctime(time.gmtime())))
            ...
            for latency in destination['blocklatency']:
                for flatency in latency['filelatency']:
                    if not flatency['time_at_destination']:
                        print flatency['lfn'], "is not coming out from", flatency["from_node"], "to", destination[
                            'name'], "since", time.asctime(
                            time.gmtime(flatency['time_first_attempt'])), "since last", time.asctime(
                            time.gmtime(flatency['time_latest_attempt'])), "after", flatency['attempts'], "attempts"
                        ret.append({
                            'file': flatency['lfn'],
    ...
    for dataset in result['phedex']['dataset']:
        for item in dataset['block']:
            ...
            if not exist:
                ...
                ## go deeper then
                r1 = conn.request("GET", '/phedex/datasvc/json/prod/filereplicas?block=%s' % (
                item['name'].replace('#', '%23')))
                r2 = conn.getresponse()
            ...
            if sub['percent_files'] != 100:
                ...
                for subitem in item_result['phedex']['block']:
                    ...
                    if delay > 0:  # more than 7 days is way to much !!
                        print subitem[
                            'name'], ' of size', block_size_GB, '[GB] missing', block_size_GB - destination_size_GB, '[GB]'
...
def getDatasetFiles(url, dataset, without_invalid=True):
    ...
    return dbs_filenames, phedex_filenames, list(set(dbs_filenames) - set(phedex_filenames)), list(
                ...
                for pick in range(n_copies):
                    ...
                    if chunk_size:
                        ...
                        if overflow < 0:
                            ...
                            ## should I crash ?
                            print picked_site_se, "has", SI.disk[
                                                             picked_site_se] * 1024., "GB free, while trying to put an item of size", chunk_size, " that is", json.dumps(
                                item, indent=2)
                            sendEmail('possibly going over quota',
...
def makeReplicaRequest(url, site, datasets, comments, priority='normal', custodial='n', approve=False, mail=True,
                       group="DataOps"):  # priority used to be normal
    dataXML = createXML(datasets)
...
def makeMoveRequest(url, site, datasets, comments, priority='normal', custodial='n',
                    group="DataOps"):  # priority used to be normal
    dataXML = createXML(datasets)
    params = {"node": site, "data": dataXML, "group": group, "priority": priority,
...
def getWorkflows(url, status, user=None, details=False, rtype=None):
    conn = httplib.HTTPSConnection(url, cert_file=os.getenv('X509_USER_PROXY'), key_file=os.getenv('X509_USER_PROXY'))
    if rtype:
        go_to = '/reqmgr2/data/request?status=%s&request_type=%s&detail=%s' % (
        status, rtype, 'true' if details else 'false')
...
class workflowInfo:
        ...
        if summary and 'errors' in summary:
            ...
            for task, errors in all_errors.items():
                ...
                for name, codes in errors.items():
                    ...
                    all_codes.extend([(int(code), info['jobs'], name, list(set([e['type'] for e in info['errors']])),
                                       list(set([e['details'] for e in info['errors']]))) for code, info in
                                      codes.items()])
    ...
    def getGlideMon(self):
        try:
            gmon = json.loads(os.popen(
                'curl -s http://cms-gwmsmon.cern.ch/prodview/json/%s/summary' % self.request['RequestName']).read())
            return gmon
        except:
    ...
    def getSiteWhiteList(self, pickone=False, verbose=True):
        ...
        if max_blow_up > 5.:
            ...
            sites_allowed = list(
                set(sites_allowed) & set([site for site in sites_allowed if SI.cpu_pledges[site] > 4000]))
